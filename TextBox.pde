class TextBox extends TextView { //<>// //<>// //<>//
  ...
  void setTextBoxListener(TextBoxListener listener) {
  ...
  boolean keyIsPressed(int keyCode) {
  ...
  void handleKeyReleasedEvent(int releasedKeyCode, char releasedKey) {
  ...
  void addInputChar(int pos, char pressedKey) {
    String buffer = "";
    if (pos > 0) {
  ...
  void deleteInputChar(int pos, int dir) {
  ...
  void setClicked(boolean clicked, int cursorPosX) {
  ...
  int calcClosestCharPos(float x) {
    int idx = -1;
  ...
  float calcAlignment(String displayedText) {
    this.context.textSize(this.textSize);
    //String outputText = this.clicked ? this.input : this.text;

...
void handleKeyPressedEvent(int pressedKeyCode, char pressedKey) {
  lastKeyEvent = millis();
  ...
  switch (pressedKeyCode) {
   ...
   case 93:
    if (isKeyPressed(16) && isKeyPressed(17)) {
      ...
      if ((speed >= 0 && speed < 1) || (speed < -0.5 && speed >= -1))
        speed = speed + 0.5;
      else
        speed++;
      updateDrawSpeed();
  ...
  case 47:
    if (isKeyPressed(16) && isKeyPressed(17)) {
      ...
      if ((speed > 0.5 && speed <= 1) || (speed <= -0.5 && speed > -1))
        speed = speed - 0.5;
      else
        speed--;
      updateDrawSpeed();
...
void handleKeyReleasedEvent(int pressedKeyCode, char pressedKey) {
...
boolean isKeyPressed(int pressedKeyCode) {
  for (int i=0; i<keyCodes.size(); i++) {
...
Point addNewPoint(Point parent, float a, float[] angle, PVector w, float alpha) {
  points.add(new Point(idCount, "" + char(this.nameCounter++), parent, a, angle, w, alpha));
...
Point addNewPoint(Point parent, PVector pos, PVector w, float alpha) {
  points.add(new Point(idCount, "" + char(this.nameCounter++), parent, pos, w, alpha));
  Point point = points.get(points.size()-1);
  point.setScale(this.scale);
  point.setScaleD(this.scaleD);
  point.setDrawSpeed(this.speed);
...
Checkbox addCheckBox(String title) {
  return addCheckBox(title, false, (ArrayList<Checkbox>)null);
}

Checkbox addCheckBox(String title, boolean checked) {
  return addCheckBox(title, checked, (ArrayList<Checkbox>)null);
}

Checkbox addCheckBox(String title, Checkbox member) {
  ArrayList<Checkbox> group = new ArrayList<Checkbox>();
  group.add(member);
  return addCheckBox(title, false, group);
}

Checkbox addCheckBox(String title, ArrayList<Checkbox> group) {
  return addCheckBox(title, false, group);
}

Checkbox addCheckBox(String title, boolean checked, Checkbox member) {
  ArrayList<Checkbox> group = new ArrayList<Checkbox>();
  group.add(member);
  return addCheckBox(title, checked, group);
}

Checkbox addCheckBox(String title, boolean checked, ArrayList<Checkbox> group) {
